from flask import Flask, render_template, request, jsonify, session, Responserage
...
def get_tiktok_videos(url, item_id):    
    ...
    try:
        ...
        print(f"Quick API Outdatd Error: {e}\n{response.text}")
        return "Quick API Outdatd"
    ...
    try:
        ...
        if not found_items:
            return f"No items with id {item_id} found."
